...
public:
    ...
    void HandlePauseStateCallback(GenericOperationalError & err) override { err = mCallbackError; mPauseCalled = true; }
    void HandleResumeStateCallback(GenericOperationalError & err) override { err = mCallbackError; mResumeCalled = true; }
    void HandleStartStateCallback(GenericOperationalError & err) override { err = mCallbackError; mStartCalled = true; }
    void HandleStopStateCallback(GenericOperationalError & err) override { err = mCallbackError; mStopCalled = true; }
    ...
    void ResetCallFlags()
    {
        mPauseCalled = mResumeCalled = mStartCalled = mStopCalled = false;
    }
};

// TestableFoo — exposes protected members for testing.
...
public:
    void SetUp() override
    {
        ...
        mCluster.emplace(kTestEndpoint, &mDelegate);
        mTester.emplace(*mCluster);
...
protected:
    ...
    std::optional<TestableOperationalStateCluster> mCluster;
    std::optional<Testing::ClusterTester> mTester;
};
...
TEST_F(OperationalStateClusterTest, InvokeStart_FromStopped_CallsDelegate)
{
    ...
    EXPECT_TRUE(result.status.has_value() || result.response.has_value());
...
TEST_F(OperationalStateClusterTest, InvokePause_FromStopped_ErrorInvalidState)
{
    ...
    {
        EXPECT_EQ(result.response->commandResponseState.errorStateID,
                  to_underlying(ErrorStateEnum::kCommandInvalidInState));
    }
}
